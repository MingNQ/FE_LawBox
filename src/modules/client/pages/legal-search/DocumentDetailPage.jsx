    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (chunksRes.success) {
          ...
          const sortedChunks = (chunksRes.result || []).sort((a, b) => a.articleNumber - b.articleNumber);
    ...
    return (
      <ClientLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <p className="text-slate-500 font-medium">Đang tải nội dung tài liệu...</p>
        </div>
      </ClientLayout>
    );
  }
    ...
    return (
      <ClientLayout>
        <div className="max-w-4xl mx-auto px-6 py-20 text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-4">Rất tiếc!</h2>
          <p className="text-slate-500 dark:text-slate-400 mb-8">{error || "Tài liệu không tồn tại hoặc đã bị xóa."}</p>
          <button 
            onClick={() => navigate(-1)}
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-8 text-sm text-slate-500 dark:text-slate-400">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-slate-700 dark:text-slate-300">Số hiệu:</span> 
                    {document.officialNumber || "Đang cập nhật"}
                  </div>
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-blue-500" />
                    <span className="font-semibold text-slate-700 dark:text-slate-300">Ban hành:</span> 
                    {document.issuedDate ? new Date(document.issuedDate).toLocaleDateString("vi-VN") : "N/A"}
                  </div>
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-blue-500" />
                    <span className="font-semibold text-slate-700 dark:text-slate-300">Hiệu lực:</span> 
                    {document.effectiveDate ? new Date(document.effectiveDate).toLocaleDateString("vi-VN") : "N/A"}
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-[1200px] mx-auto px-6 mt-8">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <aside className="lg:w-1/4">
              <div className="sticky top-[280px] bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
                <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex items-center gap-2">
                  ...
                  <span className="font-bold text-slate-800 dark:text-white text-sm">Danh mục nội dung</span>
                </div>
                <div className="max-h-[calc(100vh-320px)] overflow-y-auto p-2 space-y-1 custom-scrollbar">
                  {chunks.length > 0 ? (
                    chunks.map((chunk) => (
                      ...
                      >
                        <span className="truncate">Điều {chunk.articleNumber}</span>
                        {activeArticle === chunk.articleNumber && <ChevronRight className="w-4 h-4" />}
                      </button>
                    ))
                  ) : (
            ...
            <main className="lg:w-3/4">
              {currentChunk ? (
                <div 
                  className="bg-white dark:bg-slate-900 rounded-2xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm animate-fade-in"
                >
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center border border-slate-200 dark:border-slate-800">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">Vui lòng chọn một điều luật từ danh mục để xem nội dung.</p>
                </div>
              )}
            </main>
          </div>
